...
def home(request,name):
    if request.user.is_authenticated:
        if request.method=="POST":
            ...
            st=str(start_indian_timezone)
            correct_answer_list=["n","n","n","n","n","n","n","n"]
            ans_str=str(correct_answer_list)
            time=0
            candi=str(request.user)
            print(candi)
            print(st)
            print(time)
            print(ans_str)
            savedata=SubmittedData.objects.create(candidate=candi,start_time=st,time=time,list=ans_str)
            savedata.save()
            messages.success(request,'Your time starts now.')
            return redirect(f'/question1_luf/{request.user}')            
        else:
    ...
    if request.user.is_authenticated:
        if request.method=="POST":
            ...
            if answer==correct_answer:
                u=str(request.user)
                user = get_object_or_404(SubmittedData, candidate=u)
                ans_str=user.list
                correct_answer_list=eval(ans_str)
                correct_answer_list[0]="y"
                updated_ans_string=str(correct_answer_list)
                user.list=updated_ans_string
                user.save()
...
def complete(request,name):
    if request.user.is_authenticated:
        u=str(request.user)
        user = get_object_or_404(SubmittedData, candidate=u)
        correct_answer_str=user.list
        print(correct_answer_str)
        correct_answer_list=eval(correct_answer_str)
